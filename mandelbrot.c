...
int	get_color(int iter, int max)
{
    ...
    */
	else
    {
    	...
    	int	r = (int)(9 * (1 - t) * t * t * t * 256); //　赤色成分の計算
    	int	g = (int)(15 * (1 - t) * (1 - t) * t * t * 256); // 緑色成分の計算
	...
	for (int y = 0; y < HEIGHT; y++)
    {
        for (int x = 0; x < WIDTH; x++)
        {
            ...
            */
            ...
            c_re = data->scale * (x - WIDTH / 2.0) * (4.0 / WIDTH) + data->offset_x; //実部
            c_im = data->scale * (y - HEIGHT / 2.0) * (4.0 / HEIGHT) + data->offset_y;//虚部
            // 繰り返し計算をし、発散するか確認。
            z_re = c_re;
            z_im = c_im;
            iter = 0;
